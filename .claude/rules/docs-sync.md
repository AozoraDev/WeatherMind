...
`docs/developer-zh.md`/`developer-en.md` 是人读的模块开发文档（模块职责+文件内方法作用），**agent 勿读**、不作编码依据；但**每次代码变动必须同步更新两份**（中文简体/英文，条目一一对应）。
...
不要：不读此文档作编码依据；不为它写测试、不加入 mutate 白名单；不改 `docs/` 下其他文件。
